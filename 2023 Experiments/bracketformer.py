...
with tf.device("/cpu:0"):
  ...
  class GameFormer(tf.keras.layers.Layer):##SYMMETRIC EMBEDDINGS WONT WORK BECAUSE INDICES ARE STANDARDIZED
      ...
      def call(self, inputs):
        ...
        if tf.reduce_any(self.compute_mask(inputs)):
          ...
          x = tf.zeros((tf.shape(plays)[0], self.output_d))
        return x
  ...
  if __name__ == "__main__":
    from schedule_dataset import ds, vds, p_len, s_len, t_len, VALIDATION_PROP, p_types, s_types, lk
    ...
    model.rep_stack.game_former.positional_encoder.embedding.set_embeddings(l=lk)
    ...
    try:
        model.fit(ds, epochs=50, steps_per_epoch=int((t_len/BATCH_SIZE)/(HOURS if EPOCH_PER_HOUR else 1)), callbacks=[cp_callback], validation_data=vds, validation_steps=int((VALIDATION_PROP/BATCH_SIZE * t_len)/(HOURS if EPOCH_PER_HOUR else 1)))#, validation_data=vds)
