...
with tf.device("/gpu:0"):
    ...
    class Transformer(tf.keras.Model):
        def __init__(self, *, num_layers, d_model, num_heads, dff,
            ...
            super().__init__()
            #self.mask_lay = tf.keras.layers.Masking(mask_value=-np.inf)
            self.encoder = Encoder(num_layers=num_layers, d_model=d_model,
                                num_heads=num_heads, dff=dff,
                                vocab_size=input_vocab_size,
                                dropout_rate=dropout_rate)
            self.cat = tf.keras.layers.Concatenate(axis=-2)
    ...
    class CustomSchedule(tf.keras.optimizers.schedules.LearningRateSchedule):
        ...
        def __call__(self, step):
            ...
            return tf.math.rsqrt(self.d_model) * tf.math.minimum(arg1, arg2)/50 * self.dec ** self.decs
        
        def decrement(self):
            self.decs += 1

    from new_lr_reduction import ReduceLROnPlateau
    ...
    if __name__ == "__main__":
        ...
        lr_reduce = ReduceLROnPlateau(patience=1, min_delta=.005, callback_lambda=sched.decrement)
        #if RESUME:
        #  if manager.restore_or_initialize() is not None:
        #    model.load_weights(CHECKPOINT_PATH)
        try:
            model.fit(ds, epochs=1000, steps_per_epoch=int(t_len/BATCH_SIZE), callbacks=[cp_callback, early_stop, lr_reduce], validation_data=vds, validation_steps=int(VALIDATION_PROP/BATCH_SIZE * t_len))#, validation_data=vds)
